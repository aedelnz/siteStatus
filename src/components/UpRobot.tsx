...
interface UptimeRobotMonitor { id: number; friendly_name: string; url: string; custom_uptime_ranges: string; logs: Log[]; status: number; }

interface PostDataBody { api_key: string; format: string; logs: number; log_types: string; logs_start_date: number; logs_end_date: number; custom_uptime_ranges: string; }
...
export async function GetMonitors(apikey: string, days: number) {
    ...
    const response = await fetch('https://api.uptimerobot.com/v2/getMonitors', {
        ...
        signal: AbortSignal.timeout(50000), // 设置 50 秒超时
    })
    ...
    return data.monitors.map((monitor: UptimeRobotMonitor) => {
        ...
        // 初始化每日数据
        const daily = dates.map((date, index) => ({ date, uptime: formatNumber(ranges[index]), down: { times: 0, duration: 0 } }))

        // 创建日期到索引的映射
        const dateToIndexMap = Object.fromEntries(dates.map((date, index) => [date.format('YYYYMMDD'), index]))
        ...
        return { id: monitor.id, name: monitor.friendly_name, url: monitor.url, average, daily, total, status, }
    })
}
